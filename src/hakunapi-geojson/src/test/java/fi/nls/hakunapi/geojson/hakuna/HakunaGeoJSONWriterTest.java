...
public class HakunaGeoJSONWriterTest {
    ...
    private OutputFormat format = new OutputFormatFactoryGeoJSON().create(Collections.emptyMap());
    private OutputFormat formatExact = new OutputFormatFactoryGeoJSON().create(Map.of("formatter.degrees", "0, 5, 0, 8, 0, 13"));
    ...
    public void testWriteMultiPolygon() throws Exception {
        ...
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try (SingleFeatureWriter fw = formatExact.getSingleFeatureWriter()) {
            fw.init(baos, SRIDCode.CRS84.withDimension(HakunaGeometryDimension.XYZ));
            fw.startFeature(null, null, 1L);
    ...
    public void testWriteCollectionMetadata() throws Exception {
        ...
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try (FeatureCollectionWriter fw = format.getFeatureCollectionWriter()) {
            fw.init(baos, SRIDCode.CRS84);
    ...
    public void testWriteNullGeometryFeatureCollection() throws Exception {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try (FeatureCollectionWriter fw = formatExact.getFeatureCollectionWriter()) {
            fw.startFeatureCollection(null, null);
